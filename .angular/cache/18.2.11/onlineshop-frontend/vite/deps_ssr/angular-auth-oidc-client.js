...
import {
  Router
} from "./chunk-AJNGMGJC.js";
import "./chunk-WHJPS7FY.js";
import {
  HttpClient,
  HttpErrorResponse,
  HttpHeaders,
...
} from "./chunk-TSOILODS.js";
import {
  CommonModule,
  DOCUMENT,
  isPlatformBrowser
} from "./chunk-EWFCY476.js";
import {
  APP_INITIALIZER,
  DestroyRef,
  Injectable,
  ...
  ɵɵdefineInjector,
  ɵɵdefineNgModule
} from "./chunk-D3EDJPAT.js";
import {
  require_cjs
} from "./chunk-ZL3KHTLZ.js";
import {
  __spreadProps,
  __spreadValues,
  __toESM
...
var StsConfigStaticLoader = class {
  ...
  loadConfigs() {
    if (Array.isArray(this.passedConfigs)) {
      return (0, import_rxjs2.of)(this.passedConfigs);
    }
    return (0, import_rxjs2.of)([this.passedConfigs]);
  }
};
var StsConfigHttpLoader = class {
  ...
  loadConfigs() {
    if (Array.isArray(this.configs$)) {
      return (0, import_rxjs2.forkJoin)(this.configs$);
    }
    const singleConfigOrArray = this.configs$;
    return singleConfigOrArray.pipe((0, import_operators2.map)((value) => {
...
var PublicEventsService = class _PublicEventsService {
  constructor() {
    this.notify = new import_rxjs2.ReplaySubject(1);
  }
  /**
   * Fires a new event.
   *
...
var JwtWindowCryptoService = class _JwtWindowCryptoService {
  ...
  generateCodeChallenge(codeVerifier) {
    return this.calcHash(codeVerifier).pipe((0, import_operators2.map)((challengeRaw) => this.base64UrlEncode(challengeRaw)));
  }
  generateAtHash(accessToken, algorithm) {
    return this.calcHash(accessToken, algorithm).pipe((0, import_operators2.map)((tokenHash) => {
  ...
  calcHash(valueToHash, algorithm = "SHA-256") {
    ...
    return (0, import_rxjs2.from)(this.cryptoService.getCrypto().subtle.digest(algorithm, msgBuffer)).pipe((0, import_operators2.map)((hashBuffer) => {
...
var TokenValidationService = class _TokenValidationService {
  ...
  validateSignatureIdToken(idToken, jwtkeys, configuration) {
    ...
    return (0, import_rxjs2.from)(this.jwkWindowCryptoService.importVerificationKey(key, algorithm)).pipe((0, import_operators2.mergeMap)((cryptoKey) => {
      const signature = base64url.parse(rawSignature, {
        loose: true
      });
      const verifyAlgorithm = getVerifyAlg(alg);
      return (0, import_rxjs2.from)(this.jwkWindowCryptoService.verifyKey(verifyAlgorithm, cryptoKey, signature, signingInput));
    }), (0, import_operators2.tap)((isValid) => {
      if (!isValid) {
  ...
  validateIdTokenAtHash(accessToken, atHash, idTokenAlg, configuration) {
    ...
    return this.jwtWindowCryptoService.generateAtHash("" + accessToken, sha).pipe((0, import_operators2.mergeMap)((hash) => {
      this.loggerService.logDebug(configuration, "at_hash client validation not decoded:" + hash);
      if (hash === atHash) {
        return (0, import_rxjs2.of)(true);
      } else {
        return this.jwtWindowCryptoService.generateAtHash("" + decodeURIComponent(accessToken), sha).pipe((0, import_operators2.map)((newHash) => {
...
var AuthStateService = class _AuthStateService {
  constructor() {
    ...
    this.authenticatedInternal$ = new import_rxjs2.BehaviorSubject(DEFAULT_AUTHRESULT);
  }
  get authenticated$() {
    return this.authenticatedInternal$.asObservable().pipe((0, import_operators2.distinctUntilChanged)());
  }
  ...
  isAuthenticated(configuration) {
    if (!configuration) {
      (0, import_rxjs2.throwError)(() => new Error("Please provide a configuration before setting up the module"));
      return false;
    }
...
var UrlService = class _UrlService {
  ...
  getRefreshSessionSilentRenewUrl(config, customParams) {
    ...
    return (0, import_rxjs2.of)(this.createUrlImplicitFlowWithSilentRenew(config, customParams));
  }
  ...
  getAuthorizeUrl(config, authOptions) {
    ...
    return (0, import_rxjs2.of)(this.createUrlImplicitFlowAuthorize(config, authOptions) || "");
  }
  ...
  createBodyForParCodeFlowRequest(configuration, authOptions) {
    ...
    return this.jwtWindowCryptoService.generateCodeChallenge(codeVerifier).pipe((0, import_operators2.map)((codeChallenge) => {
      const {
        clientId,
        responseType,
        scope,
  ...
  createUrlCodeFlowAuthorize(config, authOptions) {
    ...
    return this.getCodeChallenge(config).pipe((0, import_operators2.map)((codeChallenge) => {
...
var CodeFlowCallbackHandlerService = class _CodeFlowCallbackHandlerService {
  ...
  codeFlowCallback(urlToCheck, config) {
    ...
    if (!state) {
      ...
      return (0, import_rxjs2.throwError)(() => new Error("no state in url"));
    }
    if (!code) {
      this.loggerService.logDebug(config, "no code in url");
      return (0, import_rxjs2.throwError)(() => new Error("no code in url"));
    }
    ...
    return (0, import_rxjs2.of)(initialCallbackContext);
  }
  ...
  codeFlowCodeRequest(callbackContext, config) {
    ...
    if (!isStateCorrect) {
      return (0, import_rxjs2.throwError)(() => new Error("codeFlowCodeRequest incorrect state"));
    }
    const authWellknownEndpoints = this.storagePersistenceService.read("authWellKnownEndPoints", config);
    const tokenEndpoint = authWellknownEndpoints?.tokenEndpoint;
    if (!tokenEndpoint) {
      return (0, import_rxjs2.throwError)(() => new Error("Token Endpoint not defined"));
    }
    let headers = new HttpHeaders();
    ...
    return this.dataService.post(tokenEndpoint, bodyForCodeFlow, config, headers).pipe((0, import_operators2.switchMap)((response) => {
      if (response) {
      ...
      return (0, import_rxjs2.of)(callbackContext);
    }), (0, import_operators2.retryWhen)((error) => this.handleRefreshRetry(error, config)), (0, import_operators2.catchError)((error) => {
      const {
        authority
      } = config;
      const errorMessage = `OidcService code request ${authority}`;
      this.loggerService.logError(config, errorMessage, error);
      return (0, import_rxjs2.throwError)(() => new Error(errorMessage));
    }));
  }
  handleRefreshRetry(errors, config) {
    return errors.pipe((0, import_operators2.mergeMap)((error) => {
      if (isNetworkError(error)) {
        ...
        return (0, import_rxjs2.timer)((refreshTokenRetryInSeconds ?? 0) * 1e3);
      }
...
var UserService = class _UserService {
  constructor() {
    this.userDataInternal$ = new import_rxjs2.BehaviorSubject(DEFAULT_USERRESULT);
    this.loggerService = inject(LoggerService);
    this.tokenHelperService = inject(TokenHelperService);
    this.flowHelper = inject(FlowHelper);
  ...
  getAndPersistUserDataInStore(currentConfiguration, allConfigs, isRenewProcess = false, idToken, decodedIdToken) {
    ...
    if (!(isCurrentFlowImplicitFlowWithAccessToken || isCurrentFlowCodeFlow)) {
      ...
      return (0, import_rxjs2.of)(decodedIdToken);
    }
    const {
      renewUserInfoAfterTokenRenew
    } = currentConfiguration;
    if (!isRenewProcess || renewUserInfoAfterTokenRenew || !haveUserData) {
      return this.getUserDataOidcFlowAndSave(decodedIdToken.sub, currentConfiguration, allConfigs).pipe((0, import_operators2.switchMap)((userData) => {
        ...
        if (!!userData) {
          ...
          return (0, import_rxjs2.of)(userData);
        } else {
          return (0, import_rxjs2.throwError)(() => new Error("Received no user data, request failed"));
        }
      }));
    }
    return (0, import_rxjs2.of)(existingUserDataFromStorage);
  }
  getUserDataFromStore(currentConfiguration) {
    if (!currentConfiguration) {
      return (0, import_rxjs2.throwError)(() => new Error("Please provide a configuration before setting up the module"));
    }
  ...
  getUserDataOidcFlowAndSave(idTokenSub, currentConfiguration, allConfigs) {
    return this.getIdentityUserData(currentConfiguration).pipe((0, import_operators2.map)((data) => {
  ...
  getIdentityUserData(currentConfiguration) {
    ...
    if (!authWellKnownEndPoints) {
      ...
      return (0, import_rxjs2.throwError)(() => new Error("authWellKnownEndpoints is undefined"));
    }
    ...
    if (!userInfoEndpoint) {
      ...
      return (0, import_rxjs2.throwError)(() => new Error("authWellKnownEndpoints.userinfo_endpoint is undefined"));
    }
    return this.oidcDataService.get(userInfoEndpoint, currentConfiguration, token).pipe((0, import_operators2.retry)(2));
  }
...
var SigninKeyDataService = class _SigninKeyDataService {
  ...
  getSigningKeys(currentConfiguration) {
    ...
    if (!jwksUri) {
      ...
      return (0, import_rxjs2.throwError)(() => new Error(error));
    }
    this.loggerService.logDebug(currentConfiguration, "Getting signinkeys from ", jwksUri);
    return this.dataService.get(jwksUri, currentConfiguration).pipe((0, import_operators2.retry)(2), (0, import_operators2.catchError)((e) => this.handleErrorGetSigningKeys(e, currentConfiguration)));
  }
  handleErrorGetSigningKeys(errorResponse, currentConfiguration) {
    ...
    return (0, import_rxjs2.throwError)(() => new Error(errMsg));
  }
  static {
...
var HistoryJwtKeysCallbackHandlerService = class _HistoryJwtKeysCallbackHandlerService {
  ...
  callbackHistoryAndResetJwtKeys(callbackContext, config, allConfigs) {
    ...
    return this.signInKeyDataService.getSigningKeys(config).pipe((0, import_operators2.tap)((jwtKeys) => this.storeSigningKeys(jwtKeys, config)), (0, import_operators2.catchError)((err) => {
      ...
      if (!!storedJwtKeys) {
        ...
        return (0, import_rxjs2.of)(storedJwtKeys);
      }
      return (0, import_rxjs2.throwError)(() => new Error(err));
    }), (0, import_operators2.switchMap)((jwtKeys) => {
      if (jwtKeys) {
...
var RefreshTokenCallbackHandlerService = class _RefreshTokenCallbackHandlerService {
  ...
  refreshTokensRequestTokens(callbackContext, config, customParamsRefresh) {
    ...
    return this.dataService.post(tokenEndpoint, data, config, headers).pipe((0, import_operators2.switchMap)((response) => {
...
var StateValidationService = class _StateValidationService {
  ...
  getValidatedStateResult(callbackContext, configuration) {
    ...
    if (!hasCallbackContext || hasError) {
      return (0, import_rxjs2.of)(new StateValidationResult("", "", false, {}));
    }
  ...
  validateState(callbackContext, configuration) {
    ...
    if (!this.tokenValidationService.validateStateFromHashCallback(callbackContext.authResult?.state, authStateControl, configuration)) {
      ...
      return (0, import_rxjs2.of)(toReturn);
    }
    ...
    if (hasIdToken) {
      ...
      return this.tokenValidationService.validateSignatureIdToken(toReturn.idToken, callbackContext.jwtKeys, configuration).pipe((0, import_operators2.mergeMap)((isSignatureIdTokenValid) => {
    ...
    if (callbackContext.authResult?.id_token) {
      ...
      if (isCurrentFlowCodeFlow && !toReturn.decodedIdToken.at_hash) {
        ...
        return this.tokenValidationService.validateIdTokenAtHash(
          ...
          // 'RS256'
          configuration
        ).pipe((0, import_operators2.map)((valid) => {
...
var StateValidationCallbackHandlerService = class _StateValidationCallbackHandlerService {
  ...
  callbackStateValidation(callbackContext, configuration, allConfigs) {
    return this.stateValidationService.getValidatedStateResult(callbackContext, configuration).pipe((0, import_operators2.map)((validationResult) => {
...
var UserCallbackHandlerService = class _UserCallbackHandlerService {
  ...
  callbackUser(callbackContext, configuration, allConfigs) {
    ...
    return this.userService.getAndPersistUserDataInStore(configuration, allConfigs, isRenewProcess, validationResult?.idToken, validationResult?.decodedIdToken).pipe((0, import_operators2.switchMap)((userData) => {
      if (!!userData) {
        if (!refreshToken) {
          this.flowsDataService.setSessionState(authResult?.session_state, configuration);
        }
...
var FlowsService = class _FlowsService {
  ...
  processCodeFlowCallback(urlToCheck, config, allConfigs) {
    return this.codeFlowCallbackHandlerService.codeFlowCallback(urlToCheck, config).pipe((0, import_operators2.concatMap)((callbackContext) => this.codeFlowCallbackHandlerService.codeFlowCodeRequest(callbackContext, config)), (0, import_operators2.concatMap)((callbackContext) => this.historyJwtKeysCallbackHandlerService.callbackHistoryAndResetJwtKeys(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.stateValidationCallbackHandlerService.callbackStateValidation(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.userHandlerService.callbackUser(callbackContext, config, allConfigs)));
  }
  processSilentRenewCodeFlowCallback(firstContext, config, allConfigs) {
    return this.codeFlowCallbackHandlerService.codeFlowCodeRequest(firstContext, config).pipe((0, import_operators2.concatMap)((callbackContext) => this.historyJwtKeysCallbackHandlerService.callbackHistoryAndResetJwtKeys(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.stateValidationCallbackHandlerService.callbackStateValidation(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.userHandlerService.callbackUser(callbackContext, config, allConfigs)));
  }
  processImplicitFlowCallback(config, allConfigs, hash) {
    return this.implicitFlowCallbackHandlerService.implicitFlowCallback(config, allConfigs, hash).pipe((0, import_operators2.concatMap)((callbackContext) => this.historyJwtKeysCallbackHandlerService.callbackHistoryAndResetJwtKeys(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.stateValidationCallbackHandlerService.callbackStateValidation(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.userHandlerService.callbackUser(callbackContext, config, allConfigs)));
  }
  processRefreshToken(config, allConfigs, customParamsRefresh) {
    return this.refreshSessionCallbackHandlerService.refreshSessionWithRefreshTokens(config).pipe((0, import_operators2.concatMap)((callbackContext) => this.refreshTokenCallbackHandlerService.refreshTokensRequestTokens(callbackContext, config, customParamsRefresh)), (0, import_operators2.concatMap)((callbackContext) => this.historyJwtKeysCallbackHandlerService.callbackHistoryAndResetJwtKeys(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.stateValidationCallbackHandlerService.callbackStateValidation(callbackContext, config, allConfigs)), (0, import_operators2.concatMap)((callbackContext) => this.userHandlerService.callbackUser(callbackContext, config, allConfigs)));
  }
  static {
...
var CodeFlowCallbackService = class _CodeFlowCallbackService {
  ...
  authenticatedCallbackWithCode(urlToCheck, config, allConfigs) {
    ...
    return this.flowsService.processCodeFlowCallback(urlToCheck, config, allConfigs).pipe((0, import_operators2.tap)((callbackContext) => {
...
var ImplicitFlowCallbackService = class _ImplicitFlowCallbackService {
  ...
  authenticatedImplicitFlowCallback(config, allConfigs, hash) {
    ...
    const postLoginRoute = config.postLoginRoute ?? "";
    const unauthorizedRoute = config.unauthorizedRoute ?? "";
    return this.flowsService.processImplicitFlowCallback(config, allConfigs, hash).pipe((0, import_operators2.tap)((callbackContext) => {
      if (!triggerAuthorizationResultEvent && !callbackContext.isRenewProcess) {
        this.router.navigateByUrl(postLoginRoute);
      }
    }), (0, import_operators2.catchError)((error) => {
...
var CallbackService = class _CallbackService {
  constructor() {
    ...
    this.stsCallbackInternal$ = new import_rxjs2.Subject();
  }
  get stsCallback$() {
  ...
  handleCallbackAndFireEvents(currentCallbackUrl, config, allConfigs) {
    let callback$ = new import_rxjs2.Observable();
    if (this.flowHelper.isCurrentFlowCodeFlow(config)) {
    ...
    return callback$.pipe((0, import_operators2.tap)(() => this.stsCallbackInternal$.next()));
  }
  static {
...
var AuthWellKnownDataService = class _AuthWellKnownDataService {
  ...
  getWellKnownEndPointsForConfig(config) {
    ...
    return this.getWellKnownDocument(authWellknownEndpointUrl, config).pipe((0, import_operators2.map)((wellKnownEndpoints) => ({
    ...
    })), (0, import_operators2.map)((mappedWellKnownEndpoints) => __spreadValues(__spreadValues({}, mappedWellKnownEndpoints), authWellknownEndpoints)));
  }
  getWellKnownDocument(wellKnownEndpoint, config) {
    ...
    return this.http.get(url, config).pipe((0, import_operators2.retry)(2));
  }
  static {
...
var AuthWellKnownService = class _AuthWellKnownService {
  ...
  queryAndStoreAuthWellKnownEndPoints(config) {
    ...
    return this.dataService.getWellKnownEndPointsForConfig(config).pipe((0, import_operators2.tap)((mappedWellKnownEndpoints) => this.storeWellKnownEndpoints(config, mappedWellKnownEndpoints)), (0, import_operators2.catchError)((error) => {
...
var ConfigurationService = class _ConfigurationService {
  ...
  getOpenIDConfiguration(configId) {
    if (this.configsAlreadySaved()) {
      return (0, import_rxjs2.of)(this.getConfig(configId));
    }
    return this.getOpenIDConfigurations(configId).pipe((0, import_operators2.map)((result) => result.currentConfig));
  }
  getOpenIDConfigurations(configId) {
    return this.loadConfigs().pipe((0, import_operators2.concatMap)((allConfigs) => this.prepareAndSaveConfigs(allConfigs)), (0, import_operators2.map)((allPreparedConfigs) => ({
  ...
  prepareAndSaveConfigs(passedConfigs) {
    ...
    const as = (0, import_rxjs2.forkJoin)(allHandleConfigs$).pipe((0, import_operators2.map)((config) => config.filter((conf) => Boolean(conf))), (0, import_operators2.map)((c) => c));
    return as;
  }
  ...
  handleConfig(passedConfig) {
    ...
    return (0, import_rxjs2.of)(usedConfig);
  }
...
var SilentRenewService = class _SilentRenewService {
  constructor() {
    this.refreshSessionWithIFrameCompletedInternal$ = new import_rxjs2.Subject();
    this.loggerService = inject(LoggerService);
  ...
  codeFlowCallbackSilentRenewIframe(urlParts, config, allConfigs) {
    ...
    if (errorParam) {
      ...
      this.flowsDataService.setNonce("", config);
      this.intervalService.stopPeriodicTokenCheck();
      return (0, import_rxjs2.throwError)(() => new Error(errorParam));
    }
    ...
    return this.flowsService.processSilentRenewCodeFlowCallback(callbackContext, config, allConfigs).pipe((0, import_operators2.catchError)((error) => {
      this.intervalService.stopPeriodicTokenCheck();
      this.resetAuthDataService.resetAuthorizationData(config, allConfigs);
      return (0, import_rxjs2.throwError)(() => new Error(error));
    }));
...
var RefreshSessionIframeService = class _RefreshSessionIframeService {
  ...
  refreshSessionWithIframe(config, allConfigs, customParams) {
    ...
    return this.urlService.getRefreshSessionSilentRenewUrl(config, customParams).pipe((0, import_operators2.switchMap)((url) => {
...
var RefreshSessionRefreshTokenService = class _RefreshSessionRefreshTokenService {
  ...
  refreshSessionWithRefreshTokens(config, allConfigs, customParamsRefresh) {
    ...
    return this.flowsService.processRefreshToken(config, allConfigs, customParamsRefresh).pipe((0, import_operators2.catchError)((error) => {
      this.resetAuthDataService.resetAuthorizationData(config, allConfigs);
      refreshTokenFailed = true;
      return (0, import_rxjs2.throwError)(() => new Error(error));
    }), (0, import_operators2.finalize)(() => refreshTokenFailed && this.intervalService.stopPeriodicTokenCheck()));
  }
  static {
...
var PeriodicallyTokenCheckService = class _PeriodicallyTokenCheckService {
  ...
  startTokenValidationPeriodically(allConfigs, currentConfig) {
    ...
    const periodicallyCheck$ = this.intervalService.startPeriodicTokenCheck(refreshTimeInSeconds).pipe((0, import_operators2.switchMap)(() => {
      ...
      return (0, import_rxjs2.forkJoin)(objectWithConfigIdsAndRefreshEvent);
    }));
    this.intervalService.runTokenValidationRunning = periodicallyCheck$.pipe((0, import_operators2.catchError)((error) => (0, import_rxjs2.throwError)(() => new Error(error)))).subscribe({
  ...
  getRefreshEvent(config, allConfigs) {
    ...
    return refreshEvent$.pipe((0, import_operators2.catchError)((error) => {
  ...
  createRefreshEventForConfig(configuration, allConfigs) {
    ...
    return this.configurationService.getOpenIDConfiguration(configuration.configId).pipe((0, import_operators2.switchMap)((config) => {
...
var RefreshSessionService = class _RefreshSessionService {
  ...
  userForceRefreshSession(config, allConfigs, extraCustomParams) {
    ...
    return this.forceRefreshSession(config, allConfigs, extraCustomParams).pipe((0, import_operators2.tap)(() => this.flowsDataService.resetSilentRenewRunning(config)));
  }
  forceRefreshSession(config, allConfigs, extraCustomParams) {
    ...
    if (this.flowHelper.isCurrentFlowCodeFlowWithRefreshTokens(config)) {
      return this.startRefreshSession(config, allConfigs, mergedParams).pipe((0, import_operators2.map)(() => {
    ...
    return (0, import_rxjs2.forkJoin)([this.startRefreshSession(config, allConfigs, extraCustomParams), this.silentRenewService.refreshSessionWithIFrameCompleted$.pipe((0, import_operators2.take)(1))]).pipe((0, import_operators2.timeout)(timeOutTime), (0, import_operators2.retryWhen)((errors) => {
      return errors.pipe((0, import_operators2.mergeMap)((error, index) => {
        const scalingDuration = 1e3;
        const currentAttempt = index + 1;
        if (!(error instanceof import_rxjs2.TimeoutError) || currentAttempt > MAX_RETRY_ATTEMPTS) {
          return (0, import_rxjs2.throwError)(() => new Error(error));
        }
        this.loggerService.logDebug(config, `forceRefreshSession timeout. Attempt #${currentAttempt}`);
        this.flowsDataService.resetSilentRenewRunning(config);
        return (0, import_rxjs2.timer)(currentAttempt * scalingDuration);
      }));
    }), (0, import_operators2.map)(([_, callbackContext]) => {
      const isAuthenticated = this.authStateService.areAuthStorageTokensValid(config);
      if (isAuthenticated) {
        return {
  ...
  startRefreshSession(config, allConfigs, extraCustomParams) {
    ...
    return this.authWellKnownService.queryAndStoreAuthWellKnownEndPoints(config).pipe((0, import_operators2.switchMap)(() => {
      this.flowsDataService.setSilentRenewRunning(config);
      if (this.flowHelper.isCurrentFlowCodeFlowWithRefreshTokens(config)) {
...
var CheckSessionService = class _CheckSessionService {
  constructor() {
    ...
    this.checkSessionChangedInternal$ = new import_rxjs2.BehaviorSubject(false);
  }
  ...
  pollServerSession(clientId, configuration) {
    ...
    const pollServerSessionRecur = () => {
      this.init(configuration).pipe((0, import_operators2.take)(1)).subscribe(() => {
        const existingIframe = this.getExistingIframe();
...
var PopUpService = class _PopUpService {
  constructor() {
    ...
    this.popUp = null;
    this.handle = -1;
    this.resultInternal$ = new import_rxjs2.Subject();
  }
  get result$() {
...
var CheckAuthService = class _CheckAuthService {
  ...
  checkAuth(configuration, allConfigs, url) {
    ...
    if (!config) {
      return (0, import_rxjs2.throwError)(() => new Error(`could not find matching config for state ${stateParamFromUrl}`));
    }
  ...
  checkAuthMultiple(allConfigs, url) {
    ...
    return (0, import_rxjs2.forkJoin)(allChecks$);
  }
  checkAuthIncludingServer(configuration, allConfigs) {
    if (!configuration) {
      return (0, import_rxjs2.throwError)(() => new Error("Please provide a configuration before setting up the module"));
    }
    return this.checkAuthWithConfig(configuration, allConfigs).pipe((0, import_operators2.switchMap)((loginResponse) => {
      const {
        isAuthenticated
      } = loginResponse;
      if (isAuthenticated) {
        return (0, import_rxjs2.of)(loginResponse);
      }
      return this.refreshSessionService.forceRefreshSession(configuration, allConfigs).pipe((0, import_operators2.tap)((loginResponseAfterRefreshSession) => {
  ...
  checkAuthWithConfig(config, allConfigs, url) {
    if (!config) {
      ...
      return (0, import_rxjs2.of)(result);
    }
    ...
    const callback$ = isCallback ? this.callbackService.handleCallbackAndFireEvents(currentUrl, config, allConfigs) : (0, import_rxjs2.of)({});
    return callback$.pipe((0, import_operators2.map)(() => {
      const isAuthenticated = this.authStateService.areAuthStorageTokensValid(config);
  ...
  composeMultipleLoginResults(configurations, activeConfig, url) {
    ...
    return (0, import_rxjs2.forkJoin)([currentConfigResult, ...allOtherConfigResults]);
  }
  static {
...
var ParService = class _ParService {
  ...
  postParRequest(configuration, authOptions) {
    ...
    const authWellKnownEndpoints = this.storagePersistenceService.read("authWellKnownEndPoints", configuration);
    if (!authWellKnownEndpoints) {
      return (0, import_rxjs2.throwError)(() => new Error("Could not read PAR endpoint because authWellKnownEndPoints are not given"));
    }
    const parEndpoint = authWellKnownEndpoints.parEndpoint;
    if (!parEndpoint) {
      return (0, import_rxjs2.throwError)(() => new Error("Could not read PAR endpoint from authWellKnownEndpoints"));
    }
    return this.urlService.createBodyForParCodeFlowRequest(configuration, authOptions).pipe((0, import_operators2.switchMap)((data) => {
      return this.dataService.post(parEndpoint, data, configuration, headers).pipe((0, import_operators2.retry)(2), (0, import_operators2.map)((response) => {
...
var ParLoginService = class _ParLoginService {
  ...
  loginPar(configuration, authOptions) {
    ...
    this.authWellKnownService.queryAndStoreAuthWellKnownEndPoints(configuration).pipe((0, import_operators2.switchMap)(() => this.parService.postParRequest(configuration, authOptions))).subscribe((response) => {
      this.loggerService.logDebug(configuration, "par response: ", response);
  ...
  loginWithPopUpPar(configuration, allConfigs, authOptions, popupOptions) {
    ...
    return this.authWellKnownService.queryAndStoreAuthWellKnownEndPoints(configuration).pipe((0, import_operators2.switchMap)(() => this.parService.postParRequest(configuration, authOptions)), (0, import_operators2.switchMap)((response) => {
      this.loggerService.logDebug(configuration, `par response: ${response}`);
      const url = this.urlService.getAuthorizeParUrl(response.requestUri, configuration);
      this.loggerService.logDebug(configuration, "par request url: ", url);
      if (!url) {
      ...
      return this.popupService.result$.pipe((0, import_operators2.take)(1), (0, import_operators2.switchMap)((result) => {
        const {
          userClosed,
          receivedUrl
        } = result;
...
var PopUpLoginService = class _PopUpLoginService {
  ...
  loginWithPopUpStandard(configuration, allConfigs, authOptions, popupOptions) {
    ...
    this.loggerService.logDebug(configuration, "BEGIN Authorize OIDC Flow with popup, no auth data");
    return this.authWellKnownService.queryAndStoreAuthWellKnownEndPoints(configuration).pipe((0, import_operators2.switchMap)(() => this.urlService.getAuthorizeUrl(configuration, authOptions)), (0, import_operators2.tap)((authUrl) => this.popupService.openPopUp(authUrl, popupOptions, configuration)), (0, import_operators2.switchMap)(() => {
      return this.popupService.result$.pipe((0, import_operators2.take)(1), (0, import_operators2.switchMap)((result) => {
        const {
          userClosed,
          receivedUrl
        ...
        if (userClosed) {
          ...
          return (0, import_rxjs2.of)(response);
        }
        return this.checkAuthService.checkAuth(configuration, allConfigs, receivedUrl);
      }));
    }));
...
var LogoffRevocationService = class _LogoffRevocationService {
  ...
  logoffAndRevokeTokens(config, allConfigs, logoutAuthOptions) {
    ...
    if (this.storagePersistenceService.getRefreshToken(config)) {
      return this.revokeRefreshToken(config).pipe((0, import_operators2.switchMap)((_) => this.revokeAccessToken(config)), (0, import_operators2.catchError)((error) => {
        const errorMessage = `revoke token failed`;
        this.loggerService.logError(config, errorMessage, error);
        return (0, import_rxjs2.throwError)(() => new Error(errorMessage));
      }), (0, import_operators2.concatMap)(() => this.logoff(config, allConfigs, logoutAuthOptions)));
    } else {
      return this.revokeAccessToken(config).pipe((0, import_operators2.catchError)((error) => {
  ...
  sendRevokeRequest(configuration, body) {
    ...
    const headers = this.getHeaders();
    return this.dataService.post(url, body, configuration, headers).pipe((0, import_operators2.retry)(2), (0, import_operators2.switchMap)((response) => {
...
var OidcSecurityService = class _OidcSecurityService {
  ...
  preloadAuthWellKnownDocument(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.concatMap)((config) => this.authWellKnownService.queryAndStoreAuthWellKnownEndPoints(config)));
  }
  /**
  ...
  getUserData(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.userService.getUserDataFromStore(config)));
  }
  /**
  ...
  checkAuth(url, configId) {
    return this.configurationService.getOpenIDConfigurations(configId).pipe((0, import_operators2.concatMap)(({
      allConfigs,
      currentConfig
  ...
  checkAuthMultiple(url) {
    return this.configurationService.getOpenIDConfigurations().pipe((0, import_operators2.concatMap)(({
      allConfigs
  ...
  isAuthenticated(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.authStateService.isAuthenticated(config)));
  }
  /**
  ...
  getAccessToken(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.authStateService.getAccessToken(config)));
  }
  /**
   ...
   */
  getIdToken(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.authStateService.getIdToken(config)));
  }
  /**
  ...
  getRefreshToken(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.authStateService.getRefreshToken(config)));
  }
  /**
  ...
  getAuthenticationResult(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.authStateService.getAuthenticationResult(config)));
  }
  /**
  ...
  getPayloadFromIdToken(encode = false, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => {
  ...
  setState(state, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.flowsDataService.setAuthStateControl(state, config)));
  }
  /**
   ...
   */
  getState(configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.flowsDataService.getAuthStateControl(config)));
  }
  /**
  ...
  revokeAccessToken(accessToken, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.concatMap)((config) => this.logoffRevocationService.revokeAccessToken(config, accessToken)));
  }
  /**
  ...
  revokeRefreshToken(refreshToken, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.concatMap)((config) => this.logoffRevocationService.revokeRefreshToken(config, refreshToken)));
  }
  /**
  ...
  getEndSessionUrl(customParams, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((config) => this.urlService.getEndSessionUrl(config, customParams)));
  }
  /**
  ...
  getAuthorizeUrl(customParams, configId) {
    return this.configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.concatMap)((config) => this.urlService.getAuthorizeUrl(config, customParams ? {
      customParams
    } : void 0)));
  }
  static {
...
function checkAuth$1(url, configurationService, checkAuthService, autoLoginService, loginService) {
  return configurationService.getOpenIDConfiguration().pipe((0, import_operators2.switchMap)((config) => {
    const allConfigs = configurationService.getAllConfigurations();
    return checkAuthService.checkAuth(config, allConfigs).pipe((0, import_operators2.take)(1), (0, import_operators2.map)(({
      isAuthenticated
    }) => {
      if (isAuthenticated) {
...
function checkAuth(url, configurationService, authStateService, autoLoginService, loginService, authOptions, configId) {
  return configurationService.getOpenIDConfiguration(configId).pipe((0, import_operators2.map)((configuration) => {
